...
It should be noted that if your app supports small screens or light/dark mode, you may need to write
different content for different scenarios. The `render` callback function will pass the
`layout`(list/grid) and `colorMode`(light/dark) to tell you the current situation.

```tsx live=false
metaMask({
  render({ name, id, logo, colorMode, layout, onClick }) {
    return (
      <div onClick={onClick}>
        <div>{name}</div>
        <div>{logo}</div>
