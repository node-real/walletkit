...
import { Evaluate } from 'node_modules/viem/_types/types/utils';
import {
...
import { ChainNotConfiguredError, Connector, createConnector, ProviderNotFoundError } from 'wagmi';
  ...
  return createConnector<Provider, Properties, StorageItem>((config) => ({
    ...
    async getProvider() {
      ...
      if (typeof target.provider === 'function')
        provider = await target.provider(window as Window | undefined);
...
type Target = {
  ...
  provider:
    | WalletProviderFlags
    | WalletProvider
    | ((window?: Window | undefined) => Promise<WalletProvider | undefined>);
};
